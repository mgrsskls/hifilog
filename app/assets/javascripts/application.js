...

{
  ...
  menuButtons.forEach(category => category.addEventListener("click", onSubMenuToggle));
  filterCategories.forEach(category => category.addEventListener("change", onCategoryChange));
  ...
  function onSubMenuToggle({ currentTarget: button }) {
    ...
    if (button.classList.contains("MainMenu-link--mega")) {
      document.querySelectorAll("body > nav, body > main, body > footer").forEach(element => {
