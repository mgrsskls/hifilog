-- Products
SELECT
  -- Generate a deterministic UUID from the string 'Product-{id}'
  uuid_generate_v5(uuid_ns_dns(), 'product-' || products.id::text) AS id,
  products.name,
...
-- ProductVariants
SELECT
  -- Generate a deterministic UUID from the string 'Variant-{id}'
  uuid_generate_v5(uuid_ns_dns(), 'variant-' || product_variants.id::text) AS id,
  products.name,
  products.description,
