...
--
-- Same shape as product_items plus `completeness` / `specs_*`. The highlighted-specs
-- LATERAL is intentional here: contribute filters and sorts on those columns. Catalogue
-- listings use the leaner product_items view instead.
--
...
-- completeness / specs_* stay in the view because contribute filters and orders on them, so they
-- have to be computed before pagination, unlike sub_category_names.
--
...
-- dropped from the denominator rather than counted as missing. A discontinued entry adds
-- `discontinued_year` (weight 1); one still in production is never asked for it.
--
-- The specs term pays 2 points for the fraction filled plus a 1 point bonus only once the set is
-- closed, so finishing an entry beats starting one.
--
-- Variants score out of 5 on their own description and release year, plus discontinued_year when
-- they are discontinued. Specs are excluded because
...
SELECT
  ...
  (ROUND(
    100.0 * (
      ...
      + CASE WHEN products.discontinued IS TRUE THEN 1 ELSE 0 END
    )
...
SELECT
  ...
  (ROUND(
    100.0 * (
      ...
      + CASE
          WHEN product_variants.discontinued IS TRUE
            AND product_variants.discontinued_year IS NOT NULL THEN 1
          ELSE 0
        END
    ) / (5 + CASE WHEN product_variants.discontinued IS TRUE THEN 1 ELSE 0 END)
  ))::integer AS completeness
